...
const keyboard = document.querySelectorAll("[data-keyboard]")
var guess
...
{
  getStreak.innerHTML= `${streak}`
}


function currentTargetWord()
{
  const word = targetWords[Math.floor((Math.random()*targetWords.length-1) +1)]
  return word
}
...
function getAllFilledTiles()
{
  return filledTiles = guessGrid.querySelectorAll('[data-letter]')
}
...
function changeTiles(tile, index, array, guess){
    var gameWin
    const activeTiles = [...getActiveTiles()]
    ...
    tile.addEventListener("animationend", () =>{
        ...
        if(targetWord[index] == letter){
            ...
            tile.dataset.state="wronglocation"
...
function popKey(key){
    ...
    const letter = key
    const keyPressed = document.querySelector('[data-key="' + letter + '"i]')
    keyPressed.classList.add("pop")
    
    keyPressed.addEventListener("animationend", ()=> {
        keyPressed.classList.remove("pop")
    }, {once:true})
}

function didUserWin(guess, tiles)
{
  if(guess==targetWord)
  {
    showAlert(guess+" is the correct word!")
    popTiles(tiles) 
    tiles.forEach(tile => {
      tile.addEventListener("animationend", () =>{
          clearGrid()
...
function popTiles(tiles)
{
  tiles.forEach(tile => {
    tile.classList.add("popUp")
    tile.addEventListener("animationend", () =>{
        tile.classList.remove("popUp")
    }, {once:true})
})
}
...
tile.textContent=""
tile.classList.remove("upDown")
})

allKeys.forEach(key => { 
  key.classList.remove("correct")
  key.classList.remove("wrong")
  key.classList.remove("wronglocation")
})
